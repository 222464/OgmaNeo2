...
void Predictor::learn(const Int2 &pos, std::mt19937 &rng, const IntBuffer* hiddenTargetCs) {
    ...
    for (int hc = 0; hc < _hiddenSize.z; hc++) {
        ...
        float delta = _alpha * ((hc == (*hiddenTargetCs)[hiddenColumnIndex] ? 1.0f : 0.0f) - _hiddenActivations[address3(hiddenPosition, Int2(_hiddenSize.x, _hiddenSize.y))]);
...
void Predictor::step(ComputeSystem &cs, const std::vector<const IntBuffer*> &visibleCs, const IntBuffer* hiddenTargetCs) {
    int numHiddenColumns = _hiddenSize.x * _hiddenSize.y;
    int numHidden = numHiddenColumns * _hiddenSize.z;

    // Learn
    if (hiddenTargetCs != nullptr) {
        // Learn kernel
#ifdef KERNEL_DEBUG
...
#else
        runKernel1(cs, std::bind(copyInt, std::placeholders::_1, std::placeholders::_2, visibleCs[vli], &vl._inputCsPrev), numHiddenColumns, cs._rng, cs._batchSize1);
#endif
    }
}