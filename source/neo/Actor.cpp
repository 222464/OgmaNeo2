    ...
    const Int3 &hiddenSize, int historyCapacity, const std::vector<VisibleLayerDesc> &visibleLayerDescs)
{
    ...
    for (int i = 0; i < _historySamples.size(); i++) {
        ...
        _historySamples[i]._hiddenValues = std::make_shared<FloatBuffer>(numHiddenColumns);
    }
}
...
void Actor::step(ComputeSystem &cs, const std::vector<const IntBuffer*> &visibleCs, const IntBuffer* hiddenCs, float reward, bool learnEnabled) {
...
#ifdef KERNEL_DEBUG
        for (int x = 0; x < numHiddenColumns; x++)
            copyInt(x, cs._rng, hiddenCs, s._hiddenCs.get());
#else
        runKernel1(cs, std::bind(copyInt, std::placeholders::_1, std::placeholders::_2, hiddenCs, s._hiddenCs.get()), numHiddenColumns, cs._rng, cs._batchSize1);
#endif
...
#else
        runKernel1(cs, std::bind(copyFloat, std::placeholders::_1, std::placeholders::_2, &_hiddenValues, s._hiddenValues.get()), numHiddenColumns, cs._rng, cs._batchSize1);
#endif
    ...
    if (learnEnabled && _historySize > 2) {
        ...
        for (int x = 0; x < _hiddenSize.x; x++)
            for (int y = 0; y < _hiddenSize.y; y++)
                learn(Int2(x, y), cs._rng, constGet(sPrev._visibleCs), s._hiddenCs.get(), sPrev._hiddenValues.get(), q, g);
#else
        runKernel2(cs, std::bind(Actor::learnKernel, std::placeholders::_1, std::placeholders::_2, this, constGet(sPrev._visibleCs), s._hiddenCs.get(), sPrev._hiddenValues.get(), q, g), Int2(_hiddenSize.x, _hiddenSize.y), cs._rng, cs._batchSize2);
#endif
    }
}