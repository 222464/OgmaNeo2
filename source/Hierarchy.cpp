...
void Hierarchy::forward(
    ...
    if (l == 0) {
        ...
        _rLayers[l]._activations[vli][visibleColumnIndex] = sum;//std::min(2.0f, std::max(0.0f, sum));
    }
}
...
void Hierarchy::backward(
    ...
    if (l == 0) {
        ...
        _rLayers[l + 1]._errors[0][hiddenColumnIndex] = error;//(act > 0.0f && act < 2.0f ? error : 0.0f);
    }
    else {
