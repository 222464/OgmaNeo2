        ...
        if (l == 0) {
            ...
            for (int p = 0; p < _aLayers[l].size(); p++) {
                if (inputTypes[p] == InputType::_act) {
                    ...
                    _aLayers[l][p]->init(cs, prog, inputSizes[p], aVisibleLayerDescs, rng);
                }
            }
        }
        else {
            ...
            for (int p = 0; p < _aLayers[l].size(); p++) {
                ...
                _aLayers[l][p]->init(cs, prog, layerDescs[l - 1]._hiddenSize, aVisibleLayerDescs, rng);
            }
        }
        ...
        if (_updates[l]) {
            ...
            for (int p = 0; p < _aLayers[l].size(); p++) {
                if (_aLayers[l][p] != nullptr) {
                    _aLayers[l][p]->step(cs, feedBack, rng, r, learnEnabled); //l == 0 ? inputCs[p] : _histories[l][p], 
                }
            }
        }
    }
