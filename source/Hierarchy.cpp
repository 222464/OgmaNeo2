...
void Hierarchy::forward(
    ...
    if (l == 0) {
        ...
        _rLayers[l]._activations[vli][visibleColumnIndex] = sigmoid(sum);//std::min(1.0f, std::max(0.0f, sum));
    }
}
...
void Hierarchy::backward(
    ...
    if (l == 0) {
        ...
        _rLayers[l + 1]._errors[0][hiddenColumnIndex] = error * act * (1.0f - act);//(act > 0.0f && act < 1.0f ? error : 0.0f);
    }
    else {
