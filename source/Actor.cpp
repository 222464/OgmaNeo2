...
void Actor::learn(
    ...
    if (tdErrorAction > 0.0f) {
        ...
        for (int hc = 0; hc < _hiddenSize.z; hc++) {
            ...
            float deltaAction = _beta * ((hc == targetC ? 1.0f : 0.0f) - sigmoid(sum / std::max(1, _hiddenCounts[hiddenColumnIndex])));
...
void Actor::step(
    ...
    if (learnEnabled && _historySize > 2) {
        ...
        for (int x = 0; x < _hiddenSize.x; x++)
            for (int y = 0; y < _hiddenSize.y; y++)
                learn(Int2(x, y), cs._rng, constGet(sPrev._inputCs), &s._hiddenCs, &sPrev._hiddenValues, q, g);
#else
        runKernel2(cs, std::bind(Actor::learnKernel, std::placeholders::_1, std::placeholders::_2, this, constGet(sPrev._inputCs), &s._hiddenCs, &sPrev._hiddenValues, q, g), Int2(_hiddenSize.x, _hiddenSize.y), cs._rng, cs._batchSize2);
#endif
    }
}
