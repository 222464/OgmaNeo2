...
void SparseCoder::forward(
    ...
    for (int hc = 0; hc < _hiddenSize.z; hc++) {
        ...
        for (int vli = 0; vli < _visibleLayers.size(); vli++) {
            ...
            count += vl._ffWeights.count(hiddenIndex) / vld._size.z;
        }
...
void SparseCoder::inhibit(
    ...
    for (int hc = 0; hc < _hiddenSize.z; hc++) {
        ...
        float sum = _lWeights.multiplyNoDiagonalOHVs(_hiddenCsTemp, hiddenIndex, _hiddenSize.z) / std::max(1, _lWeights.count(hiddenIndex) / _hiddenSize.z - 1); // -1 for missing diagonal
...
void SparseCoder::learn(
    ...
    for (int vli = 0; vli < _visibleLayers.size(); vli++) {
        ...
        vl._ffWeights.hebbOHVs(*inputCs[vli], hiddenIndexMax, vld._size.z, _alpha);
    }

    _lWeights.hebbOHVs(_hiddenCs, hiddenIndexMax, _hiddenSize.z, _beta);
}
...
void SparseCoder::initRandom(
    ...
    for (int vli = 0; vli < _visibleLayers.size(); vli++) {
        ...
        for (int i = 0; i < vl._ffWeights._nonZeroValues.size(); i++)
            vl._ffWeights._nonZeroValues[i] = forwardWeightDist(cs._rng);
    }
    ...
    initSMLocalRF(_hiddenSize, _hiddenSize, _lRadius, _lWeights);

    for (int i = 0; i < _lWeights._nonZeroValues.size(); i++)
        _lWeights._nonZeroValues[i] = lateralWeightDist(cs._rng);
}
