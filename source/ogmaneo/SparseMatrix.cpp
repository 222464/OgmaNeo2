...
void SparseMatrix::hebbSparseOHVs(
	const std::vector<int> &nonZeroIndices,
	int row,
	int oneHotSize,
	float alpha
) {
	float activeRatio = 1.0f / oneHotSize;
	...
	for (int jj = rowRanges[row]; jj < rowRanges[nextIndex]; jj += oneHotSize) {
		...
		for (int dj = 0; dj < oneHotSize; dj++) {
			...
			nonZeroValues[j] = std::max(0.0f, nonZeroValues[j] + alpha * (target - activeRatio));
		}
	}
}

void SparseMatrix::hebbSparseOHVsT(
	const std::vector<int> &nonZeroIndices,
	int column,
	int oneHotSize,
	float alpha
	...
	for (int jj = columnRanges[column]; jj < columnRanges[nextIndex]; jj += oneHotSize) {
		...
		for (int dj = 0; dj < oneHotSize; dj++) {
			...
			nonZeroValues[nonZeroValueIndices[j]] = std::max(0.0f, nonZeroValues[nonZeroValueIndices[j]] + alpha * (target - activeRatio));
		}
	}
}